...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	if (status)
	{
		if (status == DAQmxErrorAttributeNotSupportedInTaskContext)
			bufSize = 0; // if DAQmxGetBufInputBufSize is not supported by device (e.g USB-6001), assume unbuffered read
		else
