classdef DAQmxClass < most.APIWrapper & most.PDEPPropDynamic
    ...
    %% ABSTRACT METHOD IMPLEMENTATIONS (most.PDEPPropDynamic)
    ...
    %% ABSTRACT METHOD IMPLEMENTATIONS (most.PDEPProp)
    ...
    %% HOOK METHOD IMPLEMENTATIONS (most.APIWrapper)
    methods (Hidden)
    ...
    methods (Access=protected)
        function [apiPropTypeMap userPropNameMap lowerPropMap] = getPropertyMaps(obj)
            ...
            [apiPropTypeMap userPropNameMap lowerPropMap] = ...
                dabs.ni.daqmx.private.DAQmxClass.accessPropertyMaps(class(obj));
            ...
            if isempty(apiPropTypeMap)
                ...
                dabs.ni.daqmx.private.DAQmxClass.accessPropertyMaps(class(obj), apiPropTypeMap, userPropNameMap, lowerPropMap);
            end
...
function supportedVersions = getSupportedVersions(build)
    ...
    try
        ...
        cd(binfolder);
        cd(binfolder); % workaround for weired bug in Matlab 2015b: one single cd does not work
...
pfx86 = fullfile(most.idioms.startPath,'program files (x86)');
pf    = fullfile(most.idioms.startPath,'program files');
