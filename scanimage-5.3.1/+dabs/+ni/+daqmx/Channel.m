classdef Channel < dabs.ni.daqmx.private.DAQmxClass
            ...
            for i = 1:length(deviceNames)
                for j = 1:length(chanNameArray{i})
                    ...
                    if ~isempty(chanNameArray{i}{j}) && ~strcmpi(obj(count).chanName,chanNameArray{i}{j})
                        most.idioms.dispError(['WARNING: Specified channel name ''' chanNameArray{i}{j} ''' was not honored. ''' obj(count).chanName ''' was used instead.']);
                    end
                    
                    %Create Device object to go with channel, and bind it to Task
                    obj(count).device = dabs.ni.daqmx.Device(deviceNames{i});
