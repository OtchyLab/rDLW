classdef LinearStageController < hgsetget
    ...
    methods
        ...
        function obj = LinearStageController(varargin)
            ...
            args = most.util.filterPVArgs(varargin,{'numDeviceDimensions','positionDeviceUnits'},{'numDeviceDimensions'});
        ...
        function delete(obj)
            if most.idioms.isValidObj(obj.hMoveCompletedTimer)
    ...
    methods
        ...
        function clearSoftZero(obj,coords)
            %Set software-maintained origin (maintained by this class) to
            %the current position. coords is a 1-by-numDimensions
            %logical vec indicating which dimensions to zero. If omitted,
            %all dimensions are assumed.
            ...
            obj.relativeOrigin(coords) = 0;
            obj.zeroSoftFlag(coords) = false;
        end
        ...
        function moveStartAbsolute(obj,absTargetPos,cbkFcn)
            ...
            obj.zprvErrIfPendingNonblockingMove();
            obj.lastTargetPosition(~isnan(absTargetPos)) = absTargetPos(~isnan(absTargetPos));
            deviceAbsTargetPos = obj.zprvTargetPosPrep(absTargetPos); % throws (hardware)
    ...
    %% HDDEN METHODS - Methods for friends
    methods (Hidden)
        function changeRelativePositionTarget(obj,v)
            obj.lastTargetPosition = obj.relativeToAbsoluteCoords(v);
        end
    end
    ...
    methods (Access=private)
        ...
        function zprvErrIfPendingNonblockingMove(obj)
            if obj.nonblockingMoveInProgress
                error('Dabs:LinearStageController:SoftError',...
