classdef ZernikeGenerator < handle
    ...
    methods
        function obj = ZernikeGenerator()
            obj.initUserInterface();
        end 
    ...
    methods
        ...
        function Z = zernike(obj,n,m,res)
            ...
            if isempty(res_) || ~isequal(res,res_)
                [xx,yy] = meshgrid(linspace(-1,1,res),linspace(-1,1,res));
                [phi,r] = cart2pol(xx,yy); % this is slow! store in persistent variable for performance
                phi(abs(r)>1) = NaN;
                r(abs(r)>1) = NaN;
                res_ = res;
            end
         ...
         function B = makeBessel(obj,res,b)
             ...
             if isempty(res_) || ~isequal(res,res_)
                 [xx,yy] = meshgrid(linspace(-1,1,res),linspace(-1,1,res));
                 [~,r] = cart2pol(xx,yy); % this is slow! store in persistent variable for performance
                 res_ = res;
                 r(abs(r)>1) = NaN;
             end
    ...
    methods
        function phaseMask = composePhaseMask(obj)
            if any(obj.gradientXY)
                ...
                phaseMask = zeros(fliplr(obj.phaseMaskSizeXY));
            end
...
%                 z = z - midPoint + pi;
                ...
                phaseMaskTemp = nan(size(phaseMask));
                ...
                phaseMask(nonnanmask) = phaseMaskTemp(nonnanmask);
            end
            
            %phaseMask = phaseMask - min(phaseMask(:));
            phaseMask = mod(phaseMask,2*pi);
            
            phaseMask = phaseMask(1:obj.phaseMaskSizeXY(2),1:obj.phaseMaskSizeXY(1));
        end
    end
    
    methods (Access = private)
        function initUserInterface(obj,varargin)
            obj.hFig = figure('NumberTitle','off','Name','Zernike Generator',...
                'MenuBar','none','CloseRequestFcn',@(src,evt)delete(obj));
        ...
        function writePhaseMaskToSLM(obj)
            try
                ...
                error('Could not connect to ScanImage');
            end
            hSI.hSlmScan.writePhaseMask(obj.phaseMask);
        end        
        ...
        function updateTable(obj)            
            ...
            coefficients = mat2cell(obj.zernikeCoefficients,ones(numLines,1));
        ...
        function n = inverseFibonacci(F)
            phi = (1 + sqrt(5)) / 2;
            n = floor(log(F*sqrt(5) + 1/2)/log(phi));
        end
        ...
        function saveConfig(obj,filename)
            ...
            zernikeGenerator.phaseMaskSizeXY = obj.phaseMaskSizeXY;
    ...
    methods
        ...
        function val = get.phaseMask(obj)
            val = obj.composePhaseMask();
        end
