...
%
    ...
    if numSlicesPlusFlyback > 1 && numFrames > 1
    ...
    frameIdxs = unique(header.frameNumberAcquisition)';
    frameIdxs = frameIdxs - frameIdxs(1) + 1; % in case the file is not the first grab
    ...
    function data = loadTiff(hTif,frameIdx)
        hTif.setDirectory(frameIdx);
        data = hTif.read();
    end

    Aout = arrayfun(@(frameIdx)loadTiff(hTif,frameIdx),frameIdxs,'UniformOutput',false);
