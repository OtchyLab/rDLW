...
%                 'Position', [240 ph-233 51 22]);
        ...
        function refreshPageDependentOptions(obj)
            obj.hShutterTable.ColumnFormat = {'char' obj.hConfigEditor.availableDaqs' obj.hConfigEditor.allDigChans' {'High' 'Low'} 'numeric' 'char'};
        end
...
%                 dat = {'' '' '' '' obj.delChar};
    ...
    methods
        ...
        function cellEditFcn(obj,varargin)
            ...
            if isempty(lr) || ~isempty(lr{1}) || ~isempty(lr{2}) || ~isempty(lr{3}) || ~isempty(lr{4})
                dat(end+1,:) = {'' '' '' '' [] obj.delChar};
            end
