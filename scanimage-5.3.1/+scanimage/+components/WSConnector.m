...
%classdef WSConnectorModel < most.Model
    ...
    methods (Access = protected, Hidden)
        function componentStart(obj,acqType)
            ...
            if obj.hSI.hChannels.loggingEnable
                ...
                obj.sendWsCommands('set-data-file-folder-path',logFilePath_);
                obj.sendWsCommands('set-data-file-base-name',obj.hSI.hScan2D.logFileStem);
                
                obj.sendWsCommands('set-is-session-number-included-in-data-file-name',0);
                obj.sendWsCommands('set-is-date-included-in-data-file-name',0);
            else
                executionMode = 'play';
            end
            
            obj.sendWsCommands('set-index-of-first-sweep-in-run',obj.hSI.hScan2D.logFileCounter);
            obj.sendWsCommands('set-number-of-sweeps-in-run',numAcqs);

            obj.sendWsCommands(executionMode);            
        end
    ...
    methods
        ...
        function connect(obj)
            if obj.enable
                obj.sendWsCommands('connect',obj.PROTOCOL_VERSION,1);
    ...
    methods (Hidden)  
        ...
        function sendWsCommands(obj,cmd,param,timeout)
            ...
            if nargin>2 && ~isempty(param)
                switch class(param)
                    ...
                    case {'char'}
                        strcmd = sprintf('1\n%s| %s\n',cmd,param);
                    otherwise
                        error('Unknown parameter datatype');
                end
            else
                strcmd = sprintf('1\n%s\n',cmd);
            end
        ...
        function sendWsCommandRaw(obj,cmd,timeout)
            ...
            while toc(startTime) < timeout
                most.idioms.pauseTight(pollingInterval);
