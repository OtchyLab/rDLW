...
deviceNameAux   = '';      % String identifying NI DAQ board where digital triggers are wired
...
slmUpdateTriggerOutputTerm = '';    % Terminal on aux device, to which the SLM update trigger is connected (e.g. 'PFI1'). Leave empty if SLM does not use this trigger
...
lutCalibrationDaqDevice = '';       % Name of NI DAQ for measuring intensity of zero order spot for LUT calibration (e.g. 'PXI1Slot4'). If empty, channels from 'deviceNameAcq' are used
lutCalibrationChanID = [];          % DAQ channel for measuring intensity of zero order spot for LUT calibration (e.g. 1)