classdef SI < scanimage.interfaces.Component & most.HasMachineDataFile & most.HasClassDataFile & dynamicprops
    ...
    properties(Transient,Constant)
        ...
        VERSION_MAJOR = '5.3';     % Version number
    ...
    methods
        function cd(obj)
            ...
            p = fileparts(which('scanimage'));
            
            d = mfilename('fullpath');
            d = regexpi(d,'^[^+]*','match','once'); % remove package name
            d = regexprep(d,'\\$','');
            
            assert(strcmpi(d,p),'There might be multiple scanimage installation in the path: %s, %s',p,d);
            cd(p);
        end        
