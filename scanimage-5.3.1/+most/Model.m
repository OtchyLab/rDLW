classdef Model < handle
    ...
    %    * 'Classes': Constrain prop to one or more valid classes as defined in most.mimics.validateAttributes
    %    * 'Attributes': Constrain prop to one or more valid classes as defined in most.mimics.validateAttributes
    ...
    %       * If Class is specified as 'most.Model', the mdlPropAttributes of
    ...
    properties (Hidden)
        robotMode = false;  %Disables callbacks in all controllers while robotMode = true
    ...
    methods (Hidden)
        ...
        function obj = Model()
            ...
            function znstProcessPropAttributes()
                ...
                for i=1:length(propNames)
                    ...
                    %Processing Step 1: Store list of props specifying submodels (ie. Classes constraint is 'most.Model')s
                    if isfield(currPropMD,'Classes')
                        ...
                        if ismember('most.Model', f)
                            assert(isscalar(f) && isscalar(fieldnames(currPropMD)),'Property Class constraint to most.Model cannot be combined with other Class constraints or property attribute specification. Violated for property ''%s''.',propNames{i});
        ...
        function delete(obj)
            for c = 1:numel(obj.hController)
                most.idioms.safeDeleteObj(obj.hController{c});
                obj.hController{c}=[];
            end
        end
    ...
    methods (Access=protected, Hidden)
        ...
        function mdlInitialize(obj)
            ...
            %any associated) is the standard convention for most.Model
    ...
    methods (Access=private, Hidden)
        ...
        function ziniBindDependsOnListeners(obj)            
            propAttribs = most.util.structOrObj2List(obj.mdlPropAttributes);
            for i=1:length(propAttribs)
                ...
                if ~isempty(k)
                    ...
                    assert(~isempty(mp) && ~isempty(mp.SetMethod),'Properties with ''DependsOn'' tag specified must have a set property-access method defined (typically empty). Property ''%s'' violates this rule.',propNameFull);
                    ...
                    for j=1:length(dependsOnList)
                        ...
                        if isempty(hDdpPropPath)
                            ...
                            hDdpPropPath_ = strrep(hDdpPropPath,'.','___');
                            if isfield(obj.mdlDependencies, hDdpPropPath_) && isfield(obj.mdlDependencies.(hDdpPropPath_), dpdPropName)
                                obj.mdlDependencies.(hDdpPropPath_).(dpdPropName){end+1} = struct('mdlName', mdlName, 'mdl', hObj, 'var', propName, 'idx', numel(listenerArray) + 1);
                            else
                                obj.mdlDependencies.(hDdpPropPath_).(dpdPropName) = {struct('mdlName', mdlName, 'mdl', hObj, 'var', propName, 'idx', numel(listenerArray) + 1)};
                            end
                        end
    ...
    methods (Hidden)
        ...
        function str = mdlGetHeaderString(obj,subsetType,subsetList,numericPrecision)
            ...
            if nargin < 2 || isempty(subsetType)
                ...
                switch subsetType
                    ...
                    case 'include'
                        if ~iscolumn(subsetList)
                           subsetList = subsetList'; 
                        end
                        pnames = [obj.mdlGetHeaderableProps(); subsetList];
                    otherwise
        ...
        function s = mdlGetHeaderStruct(obj,subsetType,subsetList,numericPrecision)
            % Get string encoding of the header properties of obj.
            %   subsetType: One of {'exclude' 'include'}
            ...
            for i = 1:length(pnames)
                eval(['s.' pnames{i} '=obj.' pnames{i} ';'])
            end
            
        end

        
        function reprocessDependsOnListeners(obj, SubMdlName)
            if isfield(obj.mdlDependencies, ['hSI___' SubMdlName])
                vars = obj.mdlDependencies.(['hSI___' SubMdlName]);
                bindTos = fieldnames(vars);
                hDpdObj = obj.(SubMdlName);
                
                for i = 1:numel(bindTos)
                    bindTo = bindTos{i};
                    for j = 1:numel(vars.(bindTo))
                        bindData = vars.(bindTo){j};
                        delete(obj.mdlDependsOnListeners.([bindData.mdlName '___' bindData.var])(bindData.idx));
                        hL = hDpdObj.addlistener(bindTo,'PostSet',@(src,evnt)znstDummySet(src,evnt,bindData.mdl,bindData.var));
                        obj.mdlDependsOnListeners.([bindData.mdlName '___' bindData.var])(bindData.idx) = hL;
                    end
                end
            end
    ...
    methods (Hidden)
        ...
        % skipEqualVals (optional, default = true): if false, all properties
        % in propSet are actively set. If true, only model properties are
        % set whose value in propSet is different
        function origPropSet = mdlApplyPropSet(obj,propSet,tfOrder,skipEqualVals)
            assert(isstruct(propSet));
            ...
            if nargin < 4 || isempty(skipEqualVals)
                skipEqualVals = true;
            end
                ...
                for c = 1:numel(propNames)
                    ...
                    try
                        ...
                        if isobject(val) && ismember(pname,obj.mdlSubModelClasses)
                            ...
                            if ~skipEqualVals || ~isequal(obj.(pname),propSet.(pname))
    ...
    methods (Hidden)
        ...
        function cell = mdlGetHeaderableProps(obj,excludeSubmodels)
            propList = [setdiff(obj.zzzGetClassHeaderableProps(class(obj)),obj.mdlSubModelClasses'); obj.mdlSubModelClasses']; %Move handle-to-SubModel props to end of list
            propList = setdiff(propList, obj.mdlHeaderExcludeProps);
            ...
            for c = 1:numel(propList)
                ...
                try
                    val = obj.(pname);
                    if isobject(val)
                        if (ismember(pname,obj.mdlSubModelClasses) || isa(val,'most.Model')) && ~excludeSubmodels %Added this case for submodels.
                            hSub = val;
    ...
    methods
        function set.robotMode(obj,val)
            ...
            for idx = 1:length(obj.mdlSubModelClasses)
                obj.(obj.mdlSubModelClasses{idx}).robotMode = val;
            end
            
            for idx = 1:length(obj.hController)
                obj.hController{idx}.robotMode = val;
            end
        end
    end
    ...
    methods (Static,Hidden)
        ...
        function propNames = zzzGetClassConfigurableProps(clsName)
            ...
            % But this does not appear needed in current most.Model usage
